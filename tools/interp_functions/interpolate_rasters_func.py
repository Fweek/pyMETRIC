...
from tools.support.python_common import open_ini, read_param, remove_file, parse_int_set
...
def metric_interpolate(year_ws, ini_path, mc_iter=None, bs=None,
    ...
    Args:
        ...
        pyramids_flag (bool): if True, compute raster pyramids
    ...
    study_area_osr = gdc.feature_path_osr(study_area_path)
    ...
    study_area_extent = gdc.path_extent(study_area_path)
    ...
    env.mask_extent = gdc.project_extent(
    ...
    if mp_list:
        pool = Pool()
        results = pool.map(gdc.build_empty_raster_mp, mp_list, chunksize=1)
        pool.close()
        pool.join()
    ...
    for b_i, b_j in gdc.block_gen(env.mask_rows, env.mask_cols, bs):
        ...
        input_q.put([
            ...
            # etr_array, gdc.osr_proj(etr_osr), etr_cs, etr_extent,
            # ppt_array, gdc.osr_proj(ppt_osr), ppt_cs, ppt_extent,
    ...
    if stats_flag and mp_list:
        if mp_procs == 1:
            for raster_path in mp_list:
                gdc.raster_statistics(raster_path)
        elif mp_procs > 1:
            pool = Pool()
            results = pool.map(gdc.raster_statistics, mp_list, chunksize=1)
            pool.close()
            pool.join()
            del results, pool, mp_list
...
def block_func(block_rows, block_cols, block_extent, block_tile_list,
    ...
    if study_area_mask_flag and study_area_path and block_data_flag:
        study_area_mask_ds = gdc.polygon_to_raster_ds(
            study_area_path, nodata_value=0, burn_value=1,
            output_osr=gdc.proj_osr(snap_proj), output_cs=cellsize,
            output_extent=block_extent)
        block_data_mask = gdc.raster_ds_to_array(
