...
def load_etrf_func(array_shape, date_list, year_ws, year,
    ...
    Args:
        array_shape (list):
        # etrf_array (class:`numpy.array`): NumPy array filled with np.nan with proper shape
        date_list (list): list of dates to be processed
        year_ws (str): Filepath of the workspace to the year folder from METRIC run
        etrf_raster (str): Filepath for the output etrf_raster
        year (str): Year that will be processed
        block_tile_list (list): List of the tiles to be processed in each block
        block_extent(class:`gdal_common.env`): The gdal_common.extent of the block
        tile_image_dict (dict): A dictionary of the tiles/years to be processed
        mosaic_method (str): Mean, upper, or lower
        resampling_type (int): GDAL resampling type used to reproject the daily ETRF
        output_osr (class:`osr.SpatialReference): Desired spatial reference object
        output_cs (int): Desired cellsize of the output
        output_extent(class:`gdal_common.extent): Desired gdal_common.extent of the output
        debug_flag (bool): If True, NumPy RuntimeWarnings will be printed
    ...
    for tile_i, tile_name in enumerate(block_tile_list):
        ...
        for image_id in shuffle(tile_image_dict[year][tile_name]):
            ...
            block_tile_osr = gdc.raster_ds_osr(block_tile_ds)
            block_tile_cs = gdc.raster_ds_cellsize(block_tile_ds, x_only=True)
            block_tile_x, block_tile_y = gdc.raster_ds_origin(block_tile_ds)
            block_tile_extent = gdc.project_extent(
...
def block_interpolate_func(full_array, sub_array, sub_i_array,
    ...
    sub_bs = 64
    mp_list = []
    for s_i, s_j in gdc.block_gen(block_rows, block_cols, sub_bs):
        # logging.info('    Sub  y: {:5d}  x: {:5d}'.format(s_i, s_j))
        sub_rows, sub_cols = gdc.block_shape(
...
def load_year_array_func(input_ws, input_re, date_list,
    ...
    Args:
        ...
        output_extent ():
        name (str):
        return_geo_array (bool): if True, return array geo-spatial properties
    ...
    input_x, input_y = gdc.raster_ds_origin(input_ds)
    input_ds = None
    ...
    for date_i, date_obj in enumerate(date_list):
        try:
            ...
            continue
        output_array[date_i, :, :] = gdc.raster_to_array(
...
def unknown_proj_osr(input_proj):
    ...
    try:
        output_osr = gdc.epsg_osr(input_proj)
        logging.debug('  OSR from EPSG string')
        return output_osr
    except:
        pass
    try:
        output_osr = gdc.epsg_osr(input_proj.replace('EPSG:'))
        logging.debug('  OSR from EPSG integer')
        return output_osr
    except:
        pass
    try:
        output_osr = gdc.proj_osr(input_proj)
        logging.debug('  OSR from WKT')
        return output_osr
    except:
        pass
    try:
        output_osr = gdc.proj4_osr(input_proj)
        logging.debug('  OSR from PROJ4')
        return output_osr
    except:
        pass
    try:
        output_osr = gdc.raster_path_osr(input_proj)
        logging.debug('  OSR from raster path')
        return output_osr
    except:
        pass
    try:
        output_osr = gdc.feature_path_osr(input_proj)
