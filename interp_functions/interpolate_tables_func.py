...
from support import open_ini, read_param, parse_int_set
...
def metric_interpolate(year_ws, ini_path, mc_iter=None, bs=None,
    ...
    Args:
        year_ws (str): Year folder path
        ini_path (str): the config file (path)
        mc_iter (int): iteration number for Monte Carlo processing
        bs (int): processing block size
        overwrite_flag (bool): if True, overwrite existing files
        mp_procs (int): number of cores to use
        delay (int): max random delay starting workers in seconds
        debug_flag (bool): if True, enable debug level logging
        output_queue (int): size of output queue
    ...
    zones_osr = gdc.feature_path_osr(zones_path)
    output_osr = gdc.feature_path_osr(zones_path)
    logging.debug('  OSR: {}'.format(zones_osr))
    logging.debug('  PROJ4: {}'.format(gdc.osr_proj4(zones_osr)))

    # Get output projection from 1) zones_mask, 2) zones_proj, 3) zones_path
    # zones_osr = gdc.feature_path_osr(zones_path)
    # if zones_mask:
    #     logging.debug('  Projection from zones_mask')
    #     output_osr = gdc.raster_path_osr(zones_mask)
    # elif output_proj:
    ...
    if ((gdc.osr_proj4(output_osr) == gdc.osr_proj4(output_gcs_osr)) or
    ...
    if zones_mask:
        logging.debug('\nZones mask')
        zones_mask_osr = gdc.raster_path_osr(zones_mask)
        logging.debug('  OSR: {}'.format(zones_mask_osr))
        logging.debug('  PROJ4: {}'.format(gdc.osr_proj4(zones_mask_osr)))
        if gdc.osr_proj4(zones_mask_osr) != gdc.osr_proj4(output_osr):
            logging.warning(
    ...
    if zones_mask:
        env.cellsize = gdc.raster_path_cellsize(zones_mask, x_only=True)
        env.snap_x, env.snap_y = gdc.raster_path_origin(zones_mask)
    else:
    ...
    logging.debug('  PROJ4: {}'.format(gdc.osr_proj4(env.snap_osr)))
    ...
    for zones_ftr in zones_lyr:
        ...
        zones_extent_dict[zones_fid] = gdc.Extent(
    ...
    #     proj_extent = gdc.project_extent(
    ...
    zones_extent = gdc.union_extents(zones_extent_dict.values())
    logging.debug('  Zones extent: {}'.format(zones_extent))
    tiles_extent = gdc.union_extents([
        gdc.Extent(ogr.CreateGeometryFromWkt(
            tile_proj_wkt_dict[tile]).GetEnvelope()).ogrenv_swap()
        for tile in tile_list])
    logging.debug('  Tiles extent: {}'.format(tiles_extent))
    env.mask_extent = gdc.intersect_extents([zones_extent, tiles_extent])
    ...
    if swb_adjust_dict['flag']:
        ...
        awc_array = gdc.raster_ds_to_array(
            awc_ds, 1, awc_extent, return_nodata=False)
        awc_ds = None
    else:
        awc_osr = gdc.epsg_osr(4269)
        awc_cs = 0.125
        awc_extent = gdc.project_extent(
            env.mask_extent, env.snap_osr, ppt_osr, env.cellsize)
    ...
    # etr_array[np.isnan(etr_array)] = gdc.numpy_type_nodata(etr_array.dtype)
    # ppt_array[np.isnan(ppt_array)] = gdc.numpy_type_nodata(ppt_array.dtype)
    # awc_array[np.isnan(awc_array)] = gdc.numpy_type_nodata(awc_array.dtype)
    ...
    for b_i, b_j in gdc.block_gen(env.mask_rows, env.mask_cols, bs):
        ...
        block_extent = gdc.geo_extent(block_geo, bs, bs)
        block_extent = gdc.intersect_extents(
        ...
        zones_fid_list = sorted([
            ...
            if gdc.extents_overlap(zone_extent, block_extent)])
        ...
        input_q.put([
            ...
            # etr_array, gdc.osr_proj(etr_osr), etr_cs, etr_extent, etr_date_list,
            # ppt_array, gdc.osr_proj(ppt_osr), ppt_cs, ppt_extent, ppt_date_list,
            # awc_array, gdc.osr_proj(awc_osr), awc_cs, awc_extent,
            etr_shmem, etr_shape, gdc.osr_proj(etr_osr), etr_cs, etr_extent,
            ppt_shmem, ppt_shape, gdc.osr_proj(ppt_osr), ppt_cs, ppt_extent,
            awc_shmem, awc_shape, gdc.osr_proj(awc_osr), awc_cs, awc_extent,
...
def block_func(block_rows, block_cols, block_extent, block_tile_list,
    ...
    if calc_flags['etrf']:
        etrf_array = interp.load_etrf_func(
            ...
            gdc.proj_osr(snap_proj), cellsize, block_extent, debug_flag)
    ...
    if calc_flags['etr']:
        ...
        etr_array = gdc.project_array(
            etr_input_array, gdal.GRA_Bilinear,
            gdc.proj_osr(etr_proj), etr_cs, etr_extent,
            gdc.proj_osr(snap_proj), cellsize, block_extent)
        ...
        #     etr_array[date_i, :, :] = gdc.project_array(
    ...
    if calc_flags['ppt']:
        ...
        ppt_array = gdc.project_array(
            ppt_input_array, gdal.GRA_Bilinear,
            gdc.proj_osr(ppt_proj), ppt_cs, ppt_extent,
            gdc.proj_osr(snap_proj), cellsize, block_extent)
        # Project each PPT band/day separately
        # ppt_array = np.full(array_shape, np.nan, np.float32)
        # for date_i, date_obj in enumerate(date_list):
        #     ppt_array[date_i, :, :] = gdc.project_array(
    ...
    if swb_adjust_dict['flag']:
        ...
        awc_swb_extent = gdc.project_extent(
            block_extent, gdc.proj_osr(snap_proj),
            gdc.proj_osr(awc_proj), cellsize=cellsize)
        ...
        awc_swb_xi, awc_swb_yi = gdc.array_geo_offsets(
        ...
        etr_swb_array = gdc.project_array(
            etr_input_array, gdal.GRA_Bilinear,
            gdc.proj_osr(etr_proj), etr_cs, etr_extent,
            gdc.proj_osr(awc_proj), awc_cs, awc_swb_extent)
        ppt_swb_array = gdc.project_array(
            ppt_input_array, gdal.GRA_Bilinear,
            gdc.proj_osr(ppt_proj), ppt_cs, ppt_extent,
            gdc.proj_osr(awc_proj), awc_cs, awc_swb_extent)
        ...
        ke_swb_array = et_common.array_swb_func(
            awc_swb_array, etr_swb_array, ppt_swb_array)

        # Project Ke to Landsat spat. ref., cellsize, and extent
        ke_array = gdc.project_array(
    ...
    if zones_mask:
        ...
        zones_array, zones_nodata = gdc.raster_to_array(
    ...
    if zones_fid_list:
        ...
        zones_raster_ds.SetGeoTransform(
            gdc.extent_geo(block_extent, cs=cellsize))
