...
def pixel_points(image_ws, groupsize=1, blocksize=2048,
    ...
    Args:
        ...
        multipoing_flag (bool): if True, save cal. points to multipoint shapefile
    ...
    common_ds = gdc.raster_path_ds(image.common_area_raster, read_only=True)
    env.snap_proj = gdc.raster_ds_proj(common_ds)
    common_ds = None
...
def get_random_point_in_raster(raster_path, groupsize, blocksize):
    ...
    for b_i, b_j in gdc.block_gen_random(raster_rows, raster_cols, blocksize):
        ...
        block_array = gdc.raster_to_block(
    ...
    # raster_array = gdc.raster_ds_to_array(raster_ds, return_nodata=False)
    # raster_geo = gdc.raster_ds_geo(raster_ds)
    # raster_ds = None
    # del raster_ds
    #
    ...
    # return gdc.array_offsets_xy(raster_geo, choice(zip(xi,yi)))
