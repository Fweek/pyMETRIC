...
def pixel_rating(image_ws, ini_path, stats_flag=False, overwrite_flag=None):
    ...
    # Currently there is no tools to support applying an NLCD mask
    ...
    # Currently the tools to apply a study area mask is commented out
    ...
    # if apply_study_area_mask:
    #     study_area_mask_osr = gdc.feature_path_osr(study_area_path)
    #     study_area_mask_extent_native = gdc.path_extent(study_area_path)
    ...
    #     study_area_mask = gdc.project_array(
    ...
    #     ts_array, ts_nodata = gdc.raster_to_array(
    ...
    #     cdl_ag_mask, cdl_ag_nodata = gdc.raster_to_array(
    ...
    #     ndwi_array, ndwi_nodata = gdc.raster_to_array(
    #         ndwi_raster, 1, mask_extent=env.mask_extent)
    #     region_mask &= ndwi_array > 0.0
    ...
    # if save_dict['region_mask']:
    #     gdc.array_to_raster(region_mask, raster_dict['region_mask'])
    ...
    # # NDVI based rating
    # # Don't let NDVI be negative
    # ndvi_array, ndvi_nodata = gdc.raster_to_array(
    #     ndvi_raster, 1, mask_extent=env.mask_extent)
    ...
    #
    # # Albdo based rating
    # albedo_array, albedo_nodata = gdc.raster_to_array(
    ...
    #     cold_sugg_mask = cold_rating_array >= cold_rating_score
    #     gdc.array_to_raster(cold_sugg_mask, raster_dict['cold_sugg'])
    #     logging.debug('  Cold Percentile: {}'.format(cold_rating_pct))
    ...
    #     hot_sugg_mask = hot_rating_array >= hot_rating_score
    #     gdc.array_to_raster(hot_sugg_mask, raster_dict['hot_sugg'])
    #     logging.debug('  Hot Percentile: {}'.format(hot_rating_pct))
