...
from support import open_ini, read_param, remove_file
...
def metric_model2(image_ws, ini_path,
    ...
    Args:
        ...
        kc_cold (float): Kc value at the cold calibration point
        kc_hot (float): Kc value at the hot calibration point
        cold_xy (tuple): location of the cold calibration point
    ...
    albedo_correction_flag  = read_param(
    ...
    raster_dict['rn_24_slob'] = os.path.join(rn_ws, 'rn_24_slob' + iter_fmt)
    ...
    raster_dict['g_adjust'] = os.path.join(g_ws, 'g_adjusted' + iter_fmt)

    raster_dict['min_lai'] = os.path.join(zom_ws, 'minimum_lai' + iter_fmt)
    raster_dict['perrier_zom'] = os.path.join(zom_ws, 'perrier_zom' + iter_fmt)
    ...
    save_dict['rn_24_slob'] = read_param(
        'save_rn_24_slob_raster_flag', False, config, 'INPUTS')
    ...
    save_dict['min_lai'] = read_param(
        'save_minimum_lai_raster_flag', False, config, 'INPUTS')
    save_dict['perrier_zom'] = read_param(
        'save_perrier_zom_raster_flag', False, config, 'INPUTS')
    save_dict['excess_res'] = read_param(
    ...
    save_dict['g_adjust'] = read_param(
        'save_g_adjust_raster_flag', True, config, 'INPUTS')
    ...
    if calc_dict['h']:
        ...
        k_offset_flt = read_param('k_offset', 2.0, config, 'INPUTS')
        dt_adjust_flag = read_param('dt_adjust_flag', True, config, 'INPUTS')
        if dt_adjust_flag:
            dt_slope_factor_flt = read_param(
    ...
    if calc_dict['rah']:
        # Excess resistance can be "used" and/or saved
        calc_dict['excess_res'] = read_param(
            'use_excess_res_flag', False, config, 'INPUTS')
        # calc_dict['excess_res'] = True
    ...
    if calc_dict['zom']:
        calc_dict['landuse'] = True
        calc_dict['min_lai'] = True
        calc_dict['perrier_zom'] = True
    # Landuse specific Zom
    if calc_dict['min_lai'] or calc_dict['perrier_zom']:
            ...
            logging.error(
                ('\nERROR: The LAI reflectance type {} is invalid.' +
                 '\nERROR: Set zom_lai_refl_type to TOA or SUR').format(
                    zom_lai_refl_type))
            return False
    if calc_dict['min_lai']:
        minimum_lai_value = read_param(
            'minimum_lai_value', 0.2778, config, 'INPUTS')

    # Excess resisitance
    if calc_dict['excess_res']:
        excess_res_factor_flt = read_param(
            'excess_res_factor', 1.0, config, 'INPUTS')
        excess_res_landuse_list = read_param(
            'excess_res_landuses', [52, 71], config, 'INPUTS')
        excess_res_landuse_list = map(int, excess_res_landuse_list)
    ...
    if calc_dict['g']:
        ...
        # G adjust
        calc_dict['g_adjust'] = read_param('use_g_adjust_flag', False, config)
        if calc_dict['g_adjust']:
            g_adjust_factor = read_param('g_adjust_factor', 5.0, config)
    ...
    if calc_dict['ts_dem'] and not os.path.isfile(raster_dict['ts_dem']):
        calc_dict['ts'] = True
        calc_dict['dem'] = True
        lapse_flat_flt = read_param('lapse_flat', 6.5, config, 'INPUTS')
        lapse_mtn_flt = read_param('lapse_mtn', 10.0, config, 'INPUTS')
        lapse_elev_flt = read_param('lapse_elev', 99999.0, config, 'INPUTS')
        datum_flt = float(config.get('INPUTS', 'datum'))
    ...
    if calc_dict['landuse']:
        ...
        if (not os.path.isfile(raster_dict['landuse']) or
                gdc.raster_path_extent(raster_dict['landuse']) != env.mask_extent):
    ...
    if calc_dict['h'] or calc_dict['ts_cold_lap']:
        ...
        if cold_xy is None and os.path.isfile(cold_path):
            cold_xy = gdc.point_path_xy(cold_path)
        ...
        if hot_xy is None and os.path.isfile(hot_path):
            hot_xy = gdc.point_path_xy(hot_path)
        ...
        cold_mask = gdc.raster_value_at_xy(env.mask_path, cold_xy)
        hot_mask = gdc.raster_value_at_xy(env.mask_path, hot_xy)
    ...
    if calc_dict['excess_res'] or calc_dict['h']:
    ...
    if (calc_dict['excess_res'] or calc_dict['h']):
        ...
        # Consider setting minimum threshold on u3_flt here
        # Limit u3_flt to 0.7?
    ...
    if calc_dict['h'] or calc_dict['ts_cold_lap']:
        ...
        if ts_cold_override_flag:
            ...
            #     ts_hot_flt, elev_cold_flt, datum_flt,
            #     lapse_elev_flt, lapse_flat_flt, lapse_mtn_flt)
        ...
        if ts_hot_override_flag:
            ...
            #     ts_array[1], elev_hot_flt, datum,
            #     lapse_elev, lapse_flat, lapse_mtn)
    ...
    if calc_dict['h']:
        ...
        ts_threshold_flt = ts_dem_dry_flt + k_offset_flt
        logging.debug('    {:<14s}  {:14.8f}'.format(
            'Ts_thresh:', ts_threshold_flt))
        if dt_adjust_flag:
            logging.debug('    {:<14s}  {:14.8f}'.format(
    ...
    if calc_dict['h']:
        ...
        ts_cold_lapsed_array = et_numpy.ts_lapsed_func(
            ts_array[0], elev_array, datum_flt,
            lapse_elev_flt, lapse_flat_flt, lapse_mtn_flt)
        rs_in_array = et_numpy.rs_in_func(cos_theta_array, tau_array, image.dr)

        # Wrapped Ts correction into albedo correction function
        ...
        #     datum_flt, lapse_elev_flt, lapse_flat_flt, lapse_mtn_flt)
        # Soil Heat Flux
        ...
        # Minimum LAI
        if zom_lai_refl_type == 'TOA':
            minimum_lai_array = np.maximum(
                minimum_lai_value, lai_toa_array, dtype=np.float32)
        elif zom_lai_refl_type == 'SUR':
            minimum_lai_array = np.maximum(
                minimum_lai_value, lai_sur_array, dtype=np.float32)
        # Calculate Perrier Zom
        if zom_lai_refl_type == 'TOA':
            perrier_zom_array = et_numpy.perrier_zom_func(lai_toa_array)
        elif zom_lai_refl_type == 'SUR':
            perrier_zom_array = et_numpy.perrier_zom_func(lai_sur_array)
        # Calculate Zom
        if landuse_type == 'NLCD':
            zom_array = et_numpy.zom_nlcd_func(
                landuse_array, minimum_lai_array, perrier_zom_array)
        elif landuse_type == 'CDL':
            zom_array = et_numpy.zom_cdl_func(
                landuse_array, minimum_lai_array, perrier_zom_array)
        # Excess Resistance
        ...
        del minimum_lai_array, perrier_zom_array
            ...
            if i > 0:
                logging.info(
                    '  Updated additonal wind speed - {:4.2f} [m/s]'.format(
                        auto_wind_speed_flt))
                logging.info(
                    '  U*, U3, and excess resistance will be recalculated')
                log_fmt = '    {:<32s} {:f}'
        ...
        del excess_res_array, zom_array
    ...
    for b_i, b_j in gdc.block_gen(env.mask_rows, env.mask_cols, bs):
        ...
        if calc_dict['ts_dem'] and not os.path.isfile(raster_dict['ts_dem']):
            ...
            ts_dem_array = gdc.raster_to_block(
        ...
        if calc_dict['rn']:
            rn_array = et_numpy.rn_func(
                rs_in_array, rs_out_array, rl_in_array, rl_out_array)
        if save_dict['rn']:
            gdc.block_to_raster(rn_array, raster_dict['rn'], b_i, b_j, bs)

        # Daily Net Radiation (for evaporative fraction)
        if calc_dict['rn_24_slob']:
            lat_array, lon_array = gdc.array_lat_lon_func(
                env.snap_osr, env.cellsize, block_extent,
                gcs_cs=0.005, radians_flag=True)
            rn_24_array = et_numpy.rn_24_slob_func(
                lat_array, ts_array, ts_point_array[0], ts_point_array[1],
                albedo_sur_array, rs_in_array, image.acq_doy)
            del lat_array, lon_array
        if save_dict['rn_24_slob']:
            gdc.block_to_raster(
                rn_24_array, raster_dict['rn_24_slob'], b_i, b_j, bs)
        ...
        # Load landuse
        if calc_dict['landuse']:
            landuse_array = gdc.raster_to_array(
        ...
        if calc_dict['g_water']:
            ...
            if np.any(g_water_mask):
                g_array[g_water_mask] = et_numpy.g_water_func(
                    rn_array[g_water_mask], image.acq_doy)
        ...
        # Save G
        if save_dict['g']:
            gdc.block_to_raster(g_array, raster_dict['g'], b_i, b_j, bs)
        ...
        # Minimum LAI
        if calc_dict['min_lai'] and zom_lai_refl_type == 'TOA':
            minimum_lai_array = np.maximum(
                minimum_lai_value, lai_toa_array, dtype=np.float32)
        elif calc_dict['min_lai'] and zom_lai_refl_type == 'SUR':
            minimum_lai_array = np.maximum(
                minimum_lai_value, lai_sur_array, dtype=np.float32)
        if save_dict['min_lai']:
            gdc.block_to_raster(
                minimum_lai_array, raster_dict['min_lai'], b_i, b_j, bs)

        # Calculate Perrier Zom
        if calc_dict['perrier_zom'] and zom_lai_refl_type == 'TOA':
            perrier_zom_array = et_numpy.perrier_zom_func(lai_toa_array)
        elif calc_dict['perrier_zom'] and zom_lai_refl_type == 'SUR':
            perrier_zom_array = et_numpy.perrier_zom_func(lai_sur_array)
        if save_dict['perrier_zom']:
            gdc.block_to_raster(
                perrier_zom_array, raster_dict['perrier_zom'], b_i, b_j, bs)
        if calc_dict['lai_toa']:
            del lai_toa_array
        if calc_dict['lai_sur']:
            del lai_sur_array
        ...
        if calc_dict['zom']:
            if landuse_type == 'NLCD':
                zom_array = et_numpy.zom_nlcd_func(
                    landuse_array, minimum_lai_array, perrier_zom_array)
                # zom_array = np.empty(shape=env.mask_array.shape)
                # zom_array.fill((1.07 * 0.018))
            elif landuse_type == 'CDL':
                zom_array = et_numpy.zom_cdl_func(
                    landuse_array, minimum_lai_array, perrier_zom_array)
        if save_dict['zom']:
            gdc.block_to_raster(
                zom_array, raster_dict['zom'], b_i, b_j, bs)
        if calc_dict['min_lai']:
            del minimum_lai_array
        if calc_dict['perrier_zom']:
            del perrier_zom_array
        ...
        # Air temperature
        if calc_dict['tair']:
            tair_array = gdc.raster_to_block(
                raster_dict['tair'], b_i, b_j, bs, return_nodata=False)
        # else:
        #     tair_array = np.array([tair_flt])

        # dT (Eqn 45)
        if calc_dict['dt']:
            dt_array = et_numpy.dt_func(
                dt_adjust_flag, ts_dem_array, a, b,
                ts_threshold_flt, dt_slope_factor_flt)
            # dt_array = metric_numpy.dt_func(ts_dem_array, a_flt, b_flt)
            # if dt_adjust_flag:
            #     dt_array = metric_numpy.dt_adjust_func(
            #         dt_array, ts_dem_array, a_flt, b_flt,
            #         ts_threshold_flt, dt_slope_factor_flt)
        if save_dict['dt']:
            gdc.block_to_raster(dt_array, raster_dict['dt'], b_i, b_j, bs)
        ...
        # Save H
        if save_dict['h']:
            gdc.block_to_raster(h_array, raster_dict['h'], b_i, b_j, bs)
        ...
        # G Adjusted [W/m^2]
        # DEADBEEF - There is currently no G adjust function in et_numpy
        # if calc_dict['g_adjust']:
        #     g_array = et_numpy.g_adjust_func(
        #         g_array, ts_dem_array, ts_dem_dry_flt, g_adjust_factor)
        if save_dict['g_adjust']:
            gdc.block_to_raster(
                g_array, raster_dict['g_adjust'], b_i, b_j, bs)
        if calc_dict['ts_dem']:
            del ts_dem_array
        ...
        if save_dict['le']:
            gdc.block_to_raster(le_array, raster_dict['le'], b_i, b_j, bs)
        ...
        if save_dict['ef']:
            ef_array = et_numpy.ef_func(le_array, rn_array, g_array)
            gdc.block_to_raster(ef_array, raster_dict['ef'], b_i, b_j, bs)
        if calc_dict['et_inst']:
            del et_inst_array
        if calc_dict['rn_24_slob']:
            del rn_24_array
